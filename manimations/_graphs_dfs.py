...
class DFSExplanation(VoiceoverScene):
    def construct(self):
        ...
        title = Text("Depth-First Search (DFS) Algorithm", font_size=40)
        ...
        # Introduction
        with self.voiceover(
            "Welcome to this exploration of the Depth-First Search algorithm, or DFS for short. "
            "DFS is a fundamental graph traversal algorithm that explores as far as possible along each branch "
            "before backtracking, unlike BFS which explores level by level."
        ):
            pass
        ...
        # Create stack and visited trackers - moved more to the left
        stack_title = Text("Stack:", font_size=24).to_corner(DR).shift(LEFT * 6 + UP * 2)
        visited_title = Text("Visited:", font_size=24).to_corner(DR).shift(LEFT * 6 + UP * 0.5)
        
        stack_content = Text("[]", font_size=24).next_to(stack_title, RIGHT)
        ...
        with self.voiceover(
            ...
            "a Stack and a Visited list. Unlike BFS which uses a queue, DFS uses a stack that follows Last-In-First-Out or LIFO principle. "
            "The Stack will track nodes we need to visit next, while the Visited list will store nodes we've already processed."
        ):
            self.play(
                Write(stack_title), 
                Write(visited_title),
        ...
        with self.voiceover(
            "Since we're starting with 'Mike' as our root node, we'll add it to the stack."
        ):
            self.play(stack_content.animate.become(Text("['Mike']", font_size=20).next_to(stack_title, RIGHT)))
        
        # DFS algorithm step by step - slowed down with clearer highlighting
        with self.voiceover(
            "Now, let's start our DFS traversal. We take 'Mike' out of the stack and visit it. "
            "Then we mark it as visited and add all its unvisited neighbors to the stack. "
            "In DFS, unlike BFS, we add neighbors in reverse order since we'll process the last added node first."
        ):
            # Focus on processing the current node
            self.play(Indicate(vertices['Mike'], color=RED, scale_factor=1.5), run_time=1.5)
            
            # Update stack - remove Mike
            self.play(stack_content.animate.become(Text("[]", font_size=20).next_to(stack_title, RIGHT)), run_time=1)
            ...
            for neighbor in ['John', 'Diego', 'Mohamed']:  # Reverse order for stack
            ...
            # Add neighbors to stack one by one with animation
            self.play(stack_content.animate.become(Text("['Mohamed']", font_size=20).next_to(stack_title, RIGHT)), run_time=0.8)
            self.wait(0.3)
            self.play(stack_content.animate.become(Text("['Mohamed', 'Diego']", font_size=20).next_to(stack_title, RIGHT)), run_time=0.8)
            self.wait(0.3)
            self.play(stack_content.animate.become(Text("['Mohamed', 'Diego', 'John']", font_size=20).next_to(stack_title, RIGHT)), run_time=0.8)
            self.wait(0.5)
        
        with self.voiceover(
            "Now, following the Last-In-First-Out principle of stacks, we visit 'John' by taking it from the top of the stack. "
            "We mark it as visited and check its neighbors."
        ):
            # Focus on processing the current node
            self.play(Indicate(vertices['John'], color=RED, scale_factor=1.5), run_time=1.5)
            
            # Update stack - remove John
            self.play(stack_content.animate.become(Text("['Mohamed', 'Diego']", font_size=20).next_to(stack_title, RIGHT)), run_time=1)
            
            # Mark John as visited
            self.play(vertices['John'].animate.set_color(RED), run_time=1)
            self.play(visited_content.animate.become(Text("['Mike', 'John']", font_size=20).next_to(visited_title, RIGHT)), run_time=1)
            self.wait(0.5)
            ...
            # Visual indicator that no nodes are added to stack
            no_additions = Text("No new nodes to add", font_size=20, color=YELLOW).next_to(stack_content, DOWN)
            self.play(Write(no_additions), run_time=1)
            self.wait(1)
            self.play(FadeOut(no_additions), run_time=0.5)
        
        with self.voiceover(
            "Since John has no unvisited neighbors, we backtrack and take the next node from the stack. "
            "We visit 'Diego' by taking it from the stack, mark it as visited, and check its neighbors."
        ):
            # Focus on processing the current node
            self.play(Indicate(vertices['Diego'], color=RED, scale_factor=1.5), run_time=1.5)
            
            # Update stack - remove Diego
            self.play(stack_content.animate.become(Text("['Mohamed']", font_size=20).next_to(stack_title, RIGHT)), run_time=1)
            
            # Mark Diego as visited
            self.play(vertices['Diego'].animate.set_color(RED), run_time=1)
            self.play(visited_content.animate.become(Text("['Mike', 'John', 'Diego']", font_size=20).next_to(visited_title, RIGHT)), run_time=1)
            self.wait(0.5)
        ...
        with self.voiceover(
            "We continue to backtrack and now visit 'Mohamed' by taking it from the stack. "
            "We mark it as visited and check its neighbors."
        ):
            # Focus on processing the current node
            self.play(Indicate(vertices['Mohamed'], color=RED, scale_factor=1.5), run_time=1.5)
            
            # Update stack - remove Mohamed
            self.play(stack_content.animate.become(Text("[]", font_size=20).next_to(stack_title, RIGHT)), run_time=1)
            
            # Mark Mohamed as visited
            self.play(vertices['Mohamed'].animate.set_color(RED), run_time=1)
            self.play(visited_content.animate.become(Text("['Mike', 'John', 'Diego', 'Mohamed']", font_size=20).next_to(visited_title, RIGHT)), run_time=1)
            self.wait(0.5)
            ...
            # Reset edge color but keep Naomi yellow for stack
            self.play(
            ...
            # Add Naomi to stack
            self.play(stack_content.animate.become(Text("['Naomi']", font_size=20).next_to(stack_title, RIGHT)), run_time=1)
            self.wait(0.5)
        
        with self.voiceover(
            "Now we visit 'Naomi' by taking it from the stack. We mark it as visited and check its neighbors."
        ):
            # Focus on processing the current node
            self.play(Indicate(vertices['Naomi'], color=RED, scale_factor=1.5), run_time=1.5)
            
            # Update stack - remove Naomi
            self.play(stack_content.animate.become(Text("[]", font_size=20).next_to(stack_title, RIGHT)), run_time=1)
            
            # Mark Naomi as visited
            self.play(vertices['Naomi'].animate.set_color(RED), run_time=1)
            self.play(visited_content.animate.become(Text("['Mike', 'John', 'Diego', 'Mohamed', 'Naomi']", font_size=20).next_to(visited_title, RIGHT)), run_time=1)
            self.wait(0.5)
            ...
            # Reset edge color but keep Trevor yellow for stack
            self.play(
            ...
            # Add Trevor to stack
            self.play(stack_content.animate.become(Text("['Trevor']", font_size=20).next_to(stack_title, RIGHT)), run_time=1)
            self.wait(0.5)
        
        with self.voiceover(
            "Finally, we visit 'Trevor' by taking it from the stack. We mark it as visited and check its neighbors."
        ):
            # Focus on processing the current node
            self.play(Indicate(vertices['Trevor'], color=RED, scale_factor=1.5), run_time=1.5)
            
            # Update stack - remove Trevor
            self.play(stack_content.animate.become(Text("[]", font_size=20).next_to(stack_title, RIGHT)), run_time=1)
            
            # Mark Trevor as visited
            self.play(vertices['Trevor'].animate.set_color(RED), run_time=1)
            self.play(visited_content.animate.become(Text("['Mike', 'John', 'Diego', 'Mohamed', 'Naomi', 'Trevor']", font_size=20).next_to(visited_title, RIGHT)), run_time=1)
            self.wait(0.5)
            ...
            empty_stack = Text("Stack is empty - DFS complete", font_size=20, color=GREEN).next_to(stack_content, DOWN)
            self.play(Write(empty_stack), run_time=1)
            self.wait(1)
            self.play(FadeOut(empty_stack), run_time=0.5)
        
        # Remove stack and visited before showing the traversal order
        self.play(
            FadeOut(stack_title),
            FadeOut(stack_content),
            FadeOut(visited_title),
            FadeOut(visited_content)
        )
        
        # Show the traversal order with animation for each node
        final_order = Text("DFS Traversal Order:", font_size=28).to_edge(DOWN, buff=1)
        
        with self.voiceover(
            "And that's our final DFS traversal order. Let's review it step by step."
        ):
            self.play(Write(final_order))
            self.wait(0.5)
        
        # List of nodes in traversal order
        node_order = ['Mike', 'John', 'Diego', 'Mohamed', 'Naomi', 'Trevor']
        ...
        with self.voiceover(
            "Notice how DFS explores as far as possible along each branch before backtracking, "
            "following a different path than BFS which explores level by level."
        ):
            self.wait(2)
        ...
        insights_title = Text("Key DFS Insights", font_size=36).to_edge(UP, buff=0.5)
        
        insights = VGroup(
            Text("• DFS explores deeply along branches before backtracking", font_size=28),
            Text("• Uses a stack data structure (LIFO)", font_size=28),
            Text("• Useful for path-finding and topological sorting", font_size=28),
            Text("• Time complexity: O(V + E)", font_size=28)
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.4).center()
        
        with self.voiceover(
            "Let's highlight some key insights about DFS. First, DFS explores deeply along each branch before backtracking, "
            "using a stack data structure that follows Last-In-First-Out or LIFO principle. "
            "DFS is particularly useful for path-finding problems, maze solving, and topological sorting. "
            "Like BFS, the time complexity is O(V plus E), where V is the number of vertices "
        ...
        with self.voiceover("Here's the Python implementation of depth first search. Notice the key difference compared to BFS is that we pop from the end of the list, which acts as a stack, rather than from the beginning as in a queue. You can find the link to the full source code in the video description below.") as _:
